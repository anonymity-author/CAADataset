...
def load_audio(audio_path):
    ...
    if np.any(signal != 0):
        print("Signal contains non-zero values.")
    else:
...
def sliding_window(fkps_features, gaze_features, AUs_features, pose_features,
    ...
    if ID > 1:
        print('creating the data from the rest of the Teachers')
...
if __name__ == '__main__':
    ...
    root_dir = os.path.join(root, 'CAA-generated_database', 'train')
    create_folders(root_dir)
    np.random.seed(1)

    # read training gt file
    gt_path = './CAAM/caa_preprocessing/new_train_split.csv'
    ...
    for i in range(len(gt_df)):
     ...
     #   hog_path = f'./CAAM/data/{Teacher_ID}_P/{Teacher_ID}_hog.bin'
   ...
   #     hog_features = load_hog(hog_path)
     ...
     #   filtered_hog_features = hog_features[int(first_start_time*visual_sr):int(last_stop_time*visual_sr)]
        ...
        # audio
        filtered_audio = audio_clipping(audio, audio_sr, text_df, zero_padding=True) #12545600
